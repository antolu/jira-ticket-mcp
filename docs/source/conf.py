...
import jira_ticket_mcp

if typing.TYPE_CHECKING:
    import sphinx.application


project = "jira-ticket-mcp"
author = "Anton Lu"
version = jira_ticket_mcp.__version__

copyright = f"{datetime.datetime.now().year}, Anton Lu"  # ruff: ignore[builtin-variable-shadowing]
...
def hijack_module_name_replacement() -> None:
    ...
    import jira_ticket_mcp._mod_replace

    for mod_name in list(sys.modules):
        if mod_name == "jira_ticket_mcp._mod_replace":
            continue
        if mod_name == "jira_ticket_mcp" or mod_name.startswith("jira_ticket_mcp."):
            del sys.modules[mod_name]

    jira_ticket_mcp._mod_replace.replace_modname = lambda *_: None  # ruff: ignore[private-member-access]


hijack_module_name_replacement()


def remove__init__from_docs(  # ruff: ignore[too-many-arguments]
